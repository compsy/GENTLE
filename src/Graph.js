...
const width = (window.innerWidth > 1140 ? 1140 : window.innerWidth) - 60
...
class Graph extends Component {
   ...
   */
  callback (d, _this) {
    const nodes = JSON.parse(JSON.stringify(this.force.nodes()))
  ...
  enterCycleNodes (v3d) {
    ...
    if (this.props.opac === 'static') {
      v3d.classed('node', true)
        ...
        .style('opacity', (_d) => (0.65)) // all nodes have 0.7 opacity
    ...
    // .on("touchstart", function(d){d3.event.preventDefault(); console.log(d)})
      .on('touchstart', (_d) => { d3.event.preventDefault() })
  ...
  componentDidUpdate (prevProps, prevState, snapshot) {
    ...
    const links = JSON.parse(JSON.stringify(this.props.links))
    const foci = JSON.parse(JSON.stringify(this.props.foci))
    let conditionNode = 0
    let conditionFoci = 0
    ...
    if (conditionNode) {
      ...
      const vd3Nodes = this.virtualD3.selectAll('.node')
      ...
      if (this.props.float) {
        ...
        this.force.linkDistance(function (d, i) {
          if (window.innerWidth > 500) {
            const link = (d.source.link > d.target.link ? d.source.link * 10 : d.target.link * 10)
            if (link < 35) {
              return 35
            } else if (link > 55) {
              return 55
            ...
            const link = (d.source.link > d.target.link ? d.source.link * 3 : d.target.link * 3)
            if (link < 5) {
              return 5
            } else if (link > 10) {
              return 10
        ...
        <svg width='100%' height='75VH' ref={(e) => { this.graphRef = e }}>
          <g id='boxes'>
            {this.props.categories && (
              this.props.categories.map((category, _i) => (
